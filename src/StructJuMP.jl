...
MOI.isvalid(m::StructuredModel, vref::StructuredVariableRef) = vref.idx in keys(m.variables)
JuMP.num_variables(m::StructuredModel) = length(m.variables)

JuMP.haslowerbound(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.haslb
function JuMP.lowerbound(vref::StructuredVariableRef)
    @assert !JuMP.isfixed(vref)
    vref.model.variables[vref.idx].info.lowerbound
end
function JuMP.setlowerbound(vref::StructuredVariableRef, lower)
    vref.model.variables[vref.idx].info.haslb = true
    vref.model.variables[vref.idx].info.lowerbound = lower
end
JuMP.hasupperbound(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.hasub
function JuMP.upperbound(vref::StructuredVariableRef)
    @assert !JuMP.isfixed(vref)
    vref.model.variables[vref.idx].info.upperbound
end
function JuMP.setupperbound(vref::StructuredVariableRef, upper)
    vref.model.variables[vref.idx].info.hasub = true
    vref.model.variables[vref.idx].info.upperbound = upper
end
JuMP.isfixed(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.hasfix
JuMP.fixvalue(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.fixedvalue
function JuMP.fix(vref::StructuredVariableRef, value)
    vref.model.variables[vref.idx].info.fixedvalue = value
end
JuMP.startvalue(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.start
function JuMP.setstartvalue(vref::StructuredVariableRef, start)
    vref.model.variables[vref.idx].info.start = start
end
JuMP.isbinary(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.binary
function JuMP.setbinary(vref::StructuredVariableRef)
    @assert !JuMP.isinteger(vref)
    vref.model.variables[vref.idx].info.binary = true
end
function JuMP.unsetbinary(vref::StructuredVariableRef)
    vref.model.variables[vref.idx].info.binary = false
end
JuMP.isinteger(vref::StructuredVariableRef) = vref.model.variables[vref.idx].info.integer
function JuMP.setinteger(vref::StructuredVariableRef)
    @assert !JuMP.isbinary(vref)
    vref.model.variables[vref.idx].info.integer = true
end
function JuMP.unsetinteger(vref::StructuredVariableRef)
...
JuMP.constrainttype(::StructuredModel) = StructuredConstraintRef
...
MOI.isvalid(m::StructuredModel, cref::StructuredConstraintRef) = cref.idx in keys(m.constraints)
function JuMP.constraintobject(cref::StructuredConstraintRef, F::Type, S::Type)
...
function JuMP.setname(vref::StructuredVariableRef, name::String)
