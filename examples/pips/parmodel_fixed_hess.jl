...
@addNLConstraint(m, 1.0*x[1] + 1.1*x[2]<= 101)
@addNLConstraint(m, 2.0*x[2] + 2.2*x[3]<= 102)
@addNLConstraint(m, 3.0*x[3] + 3.3*x[4]<= 103)

for i in 1:scen
    bl = StructuredModel(parent=m)
    @defVar(bl, y[1:3])
    @addNLConstraint(bl, 1.0*x[1] + 1.1*x[2] - 3.2*y[1] + 3.3* y[2] <= 201)
    @addNLConstraint(bl, 2.0*x[2] + 2.1*x[3] - 2.2*y[2] + 2.3*y[3]  <= 202)
    @addNLConstraint(bl, 3.0*x[3] + 3.1*x[4] - 3.2*y[3]              <= 203)
