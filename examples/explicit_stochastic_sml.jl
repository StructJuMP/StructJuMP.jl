...
@defVar(m, xh0[ASSETS] >= 0)
@defVar(m, xb0[ASSETS] >= 0)
@addConstraint(m, (1+Ct)*sum{val[j]*xb0[j], j=ASSETS} <= BUDGET)

for n1 in NODES
    if PARENT[n1] == 0
        sub = Block(m, n1)
        @defVar(sub, xh1[ASSETS] >= 0)
        @defVar(sub, xb1[ASSETS] >= 0)
        @defVar(sub, xs1[ASSETS] >= 0)
        for j in ASSETS
    ...
    for n2 in NODES
        if PARENT[n2] == n1
            sub = Block(n1, n2)
            @defVar(sub, xh2[ASSETS] >= 0)
            @defVar(sub, xb2[ASSETS] >= 0)
            @defVar(sub, xs2[ASSETS] >= 0)
            for j in ASSETS
