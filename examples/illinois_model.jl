...
# lines
df = readtable("Illinois/Lines_data.tab", separator='\t', skipstart=1)
LIN          = df[:LIN]
...
# buses
df = readtable("Illinois/bus_data.tab", separator='\t', skipstart=1)
BUS     = df[:BUS]

# thermal generators
df = readtable("Illinois/Gen_data_thermals.tab", separator='\t', skipstart=1)
GENTHE       = df[:GENTHE]
...
# wind generators
df = readtable("Illinois/Gen_data_wind.tab", separator='\t', skipstart=1)
GENWIN     = df[:GENWIN]
...
# fuels
df = readtable("Illinois/fuel_data_distinctPrices.tab", separator='\t', skipstart=1)
FUEL        = df[:FUEL]
HV          = Dict(FUEL,df[:HV])
Unitprice   = Dict(FUEL,df[:Unitprice])

# loads
df = readtable("Illinois/load_load.tab", separator='\t', skipstart=1)
LOAD     = df[:LOAD]
bus_load = Dict(LOAD,df[:bus_load])

df = readdlm("Illinois/Loads.dat", '\t')
loads = df[3,:]
